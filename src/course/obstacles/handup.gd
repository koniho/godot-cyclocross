...
func _on_body_entered(body: Node2D):
	if body.is_in_group("riders") and is_available:
...
func _apply_effect(rider: Node2D):
	if not rider.is_in_group("riders"):
		return
...
func _apply_speed_boost(rider: Node2D, multiplier: float, duration: float):
...
func _apply_invincibility(rider: Node2D, duration: float):
...
func _apply_score_multiplier(rider: Node2D, multiplier: float, duration: float):
